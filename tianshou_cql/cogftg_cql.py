...
def get_args():
    ...
    parser.add_argument("--task", type=str, default="DareFightICE")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--eps-test", type=float, default=0.001)
    parser.add_argument("--lr", type=float, default=0.0001)
    parser.add_argument("--gamma", type=float, default=0.99)
    parser.add_argument("--num-quantiles", type=int, default=50)
    parser.add_argument("--n-step", type=int, default=5)
    parser.add_argument("--target-update-freq", type=int, default=10000)
    parser.add_argument("--min-q-weight", type=float, default=50.)
    parser.add_argument("--epoch", type=int, default=100)
    parser.add_argument("--update-per-epoch", type=int, default=50000)
...
def test_discrete_cql(args=get_args()):
    ...
    '''
    observation_space = spaces.Box(low=-1.9, high=1.9, shape=(800, 2))
    ...
    # model
    net = QRDQN(hidden_sizes=args.hidden_sizes, num_quantiles=args.num_quantiles, device=args.device)
    ...
    def save_checkpoint_fn(epoch: int,env_step: int, gradient_step: int):
        save_file = 'epoch_' + str(epoch) + '.pth'
        torch.save(policy.state_dict(), os.path.join(log_path, save_file))
