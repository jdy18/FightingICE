...
class FightingICEDataset(Dataset):
    def __init__(self, path, sequence_len=32, ROLL_OUT=3600):
        ...
        # self.trajectories1 = torch.load(path.replace('_k', '_1'))
    ...
    def __getitem__(self, start_idx):
        ...
        return self.trajectories['actions'][eps_idx, series_idx], self.trajectories['states'][eps_idx, series_idx], self.trajectories['rewards'][eps_idx, series_idx]

if __name__ == '__main__':
    start_time = time.time()
    datasets = FightingICEDataset('./Data_random.pth')
    end_time = time.time()
