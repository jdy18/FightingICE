...
class SoundAgent(AIInterface):
    ...
    def round_end(self, round_result: RoundResult):
        ...
        if obs is not None:
            ...
            self.collect_data_helper.put_raw_data(self.audio_data)  # 每记录一次state就记录一次音频数据
        # final step
    ...
    def processing(self):
        ...
        action = torch.randint(0, 40, [1])  # 随机生成动作
        # put to helper
...
class CollectDataHelper:
    ...
    def put_raw_data(self, raw_data):  # 收集音频数据
        data = [raw_data.raw_data_as_bytes, raw_data.fft_data, raw_data.spectrogram_data_as_bytes]
        self.current_round_raw_data.append(data)
    ...
    def finish_round(self):
        ...
        self.total_round_raw_data.append(self.current_round_raw_data)
        self.current_round_raw_data = []
