...
from encoder import MelSpecEncoder
...
class QRDQN(Net):
    ...
    def __init__(
        self,
        encode_obs_shape: Union[int, Sequence[int]] = 2560,
        action_shape: Sequence[int] = 40,
        hidden_sizes: Sequence[int] = (),
        num_quantiles: int = 200,
        device: Union[str, int, torch.device] = "cpu",
        encoder=MelSpecEncoder(frame_skip=1)
    ) -> None:
        self.action_num = np.prod(action_shape)
        super().__init__(state_shape=encode_obs_shape, action_shape=[self.action_num * num_quantiles], hidden_sizes=hidden_sizes, device=device)
    ...
    def forward(
        ...
        if self.encoder:
            ...
            encode_obs = self.relu(self.flatten(self.encoder(obs)))
        dist, state = super().forward(obs=encode_obs)
        dist = dist.view(-1, self.action_num, self.num_quantiles)
        return dist, state
    ...
    def act(
        ...
        if self.encoder:
            ...
            encode_obs = self.relu(self.flatten(self.encoder(obs)))
        dist, state = super().forward(obs=encode_obs)
        dist = dist.view(-1, self.action_num, self.num_quantiles).sum(2)
        return dist