...
def process_game_agent_data(actor_, critic_, data_collect_helper, recurrent):
    ...
    for r in range(number_round):
        ...
        for i in tqdm.trange(1, min(len(round_data) - 1, len(round_actions))):
            ...
            round_reward_list.append(torch.tensor(transformed_reward).float().to(GATHER_DEVICE))
...
def collect_trajectories(actor, critic, port, game_num, p2, rnn, n_frame):
    ...
    while error:
        ...
        try:
            ...
            agent = SoundAgent(actor=actor, critic=critic, collect_data_helper=collect_data_helper, logger=logger, n_frame=n_frame, rnn=rnn, device = TRAIN_DEVICE)
            #agent2 = SoundAgent(actor=actor, critic=critic, collect_data_helper=collect_data_helper, logger=logger, n_frame=n_frame, rnn=rnn)
            gateway.register_ai('SoundAgent', agent)
            #Qgateway.register_ai('SoundAgent', agent2)
...
def pad_and_compute_returns(trajectory_episodes, len_episodes):
    ...
    for i in range(episode_count):
        ...
        for key, value in trajectory_episodes.items():
            ...
            padded_trajectories[key].append(torch.cat((value[i], padding.to(value[i].device))))
...
def train_model(actor, critic, actor_optimizer, critic_optimizer, iteration, port, encoder_name, experiment_id, p2, recurrent, n_frame, epoch, training_iteration, game_num):
    ...
    while iteration < training_iteration:
        ...
        trajectories_data = {}
        # run 1 game in GAME_NUM times and concatenate game data together 收集游戏数据
        ...
        #normalized_stddev_reward = np.std(trajectories["rewards"].sum(axis=1).cpu().numpy())
