...
from Collect_Agent import SoundAgent, CollectDataHelper
...
def process_game_agent_data(actor_, critic_, data_collect_helper, recurrent):
    ...
    for r in range(number_round):
        ...
        round_raw_data = data_collect_helper.total_round_raw_data[r]
        ...
        for i in tqdm.trange(1, min(len(round_data) - 1, len(round_actions))):
            ...
            round_raw_data_list.append(audio)
        ...
        state = torch.tensor(obsv, dtype=torch.float32)
        round_next_state_list = round_state_list[1:].copy()+[state]
        round_raw_data_list.append(audio)
        round_terminal_list[-1] = torch.tensor(1)
        ...
        next_state_list.append(round_next_state_list)
        ...
        raw_data.append(round_raw_data_list)
    ...
    return {key: [torch.stack(v) for v in value] for key, value in trajectories_data.items()}, raw_data
...
def process_game_agent_data(actor_, critic_, data_collect_helper, recurrent):
    ...
    for (key, value)  in trajectories_data.items():
        for i in range(len(value)):
            if len(value[i]) == 0:
                logger.info(f'empty tensorlist: {key} {i}')
    return {key: [torch.stack(v) for v in value] for key, value in trajectories_data.items()}, episode_lengths, raw_data
'''
...
class TrajectoryDataset():
    ...
    def __next__(self):
        if self.batch_count * self.batch_size >= math.ceil(self.cumsum_seq_len[-1] / self.sequence_len):
            ...
            self.valid_idx = np.setdiff1d(self.valid_idx, start_idx)  # 删去已采样的样本
            eps_idx = np.digitize(start_idx, bins=self.cumsum_seq_len, right=False) - 1  # 各序列属于哪一个轨迹
            seq_idx = start_idx - self.cumsum_seq_len[eps_idx]  # 该序列开始位置在当前轨迹中的序号
            series_idx = np.linspace(seq_idx, seq_idx + self.sequence_len - 1, num=self.sequence_len, dtype=np.int64)  # 取出长度为32的序列
            self.batch_count += 1
            if self.recurrent:
...
def train_model(actor, critic, actor_optimizer, critic_optimizer, iteration, port, encoder_name, experiment_id, p2, recurrent, n_frame, epoch, training_iteration, game_num, sars_path, raw_data_path):
    loop_count = 0
    '''
    if not recurrent:
    ...
    while iteration < training_iteration:
        ...
        for i in range(game_num):
            ...
            game_trajectories_data, game_raw_data = collect_trajectories(actor, critic, port, 1, p2, recurrent, n_frame)
        ...
        for k, v in trajectories_data.items():
            trajectories_data[k] = torch.cat(v, dim=0)

        logger.info('Saving Data')  # 保存数据
        new = OrderedDict(trajectories_data)
        sars = torch.load(sars_path) if os.path.isfile(sars_path) else OrderedDict()
        # raw = np.load(raw_data_path, allow_pickle=True).tolist() if os.path.isfile(raw_data_path) else []
        if sars:
            for k, v in sars.items():
                sars[k] = torch.cat((sars[k], new[k]), dim=0)
        else:
            sars = new.copy()
        # raw += raw_data
        # raw = np.array(raw, dtype=object)
        torch.save(sars, sars_path)
        # np.save(raw_data_path, raw)
        del sars
        # del raw
...
if __name__ == '__main__':
    ...
    parser.add_argument('--training_iteration', type=int, default=15, help='Number of training iterations')
    parser.add_argument('--game_num', type=int, default=20, help='Number of games to play per iteration')
    ...
    # 加载预训练权重
    actor_state_dict = torch.load(os.path.join('../../trained_model', "actor.pt"))
    actor.load_state_dict(actor_state_dict, strict=True)
    critic_state_dict = torch.load(os.path.join('../../trained_model', "critic.pt"))
    ...
    try:
        ...
        train_model(actor, critic, actor_optim, critic_optim, iteration, args.port, args.encoder, args.id, args.p2, args.recurrent,
                    args.n_frame, args.epoch, args.training_iteration, args.game_num, sars_path='../Data_pretrain.pth', raw_data_path='../raw_pretrain.npy')
    except Exception as ex:
        print(ex)
